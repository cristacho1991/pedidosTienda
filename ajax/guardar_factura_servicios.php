...
if (!empty($factura))
{
	...
	$sql=mysqli_query($con, "select * from modfacprint where modfacprint.nrofact='$factura'");
$i=0;
	while ($row=mysqli_fetch_array($sql))
	{
		$product_id[$i]=$row['codigo'];
		$nrofact[$i]=$row['nrofact'];
		$descripcion[$i]=$row['trabajo'];
		$cant[$i]=$row['cantidad'];
		$valortot[$i]=$row['valor'];
		$includes_tax[$i]=$row['iva'];
...
var_dump(sizeof($product_id));
	...
	for($i=0;$i<sizeof($product_id);$i++){
	   ...
	   VALUES ('01','01',1,'$nrofact[$i]','$product_id[$i]','',null,'01',$cant[$i],0,0,0.00,$precio_unitario[$i],0.0000,0.00,0.00000000,$subtotal,'',0.00,0.00,$cant[$i],'F','$includes_tax[$i]',0,0,'',0.00,0.00,0)");
