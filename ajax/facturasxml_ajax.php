						...
						<div class="table-responsive">
							<table class="table  table-condensed table-hover table-striped " id="tabladin">
								<tr>
									...
									<th class='text-center'>Enviar SRI </th>
							...
							<td class=' text-center'><?php echo $tipo_doc; ?></td>
										...
										<td class='text-center' id="numero"><?php echo $num_doc; ?></td>
										<td class='text-center'><?php echo $fecha; ?></td>
										<td class='text-center'><?php echo $detalle; ?></td>
										<td class='text-center'><?php echo $valor; ?></td>
										<td class='text-center'><?php echo $email; ?></td>
										<td class='text-center'><input class="form-check-input" type="checkbox" value="<?php echo $j; ?>" name="checkxml" id="flexCheckDefault"></td>
...
//}
	...
	<script>
		$("#enviaxml").click(function() {
			...
			let nfact = [];
			let id = [];
			let cont = arrTodo.length;
			for (let i = 0; i < cont; i++) {

				if (arrTodo[i].status == 1) {
					id[i] = arrTodo[i].id;
					nfact.push(arrTodo[i].idfactura);
				}
			}
			console.log(nfact);

			let cont2 = nfact.length;
			for (let i = 0; i < cont2; i++) {
				console.log(nfact[i]);

				VentanaCentrada('./pdf/documentos/factura2.php?nfact=' + nfact[i], 'PDF', '', '1024', '768', 'true');
				VentanaCentrada('./pdf/enviaPDF/enviapdf.php?nfact=' + nfact[i], '', '1024', '768', 'true');
			...
			$.ajax({
				url: './view/facturacionxml/transforma_xml.php',
				method: "POST",
				data: toPost,
				dataType: 'text',
