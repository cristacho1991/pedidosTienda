...
$insert=mysqli_query($con,"INSERT INTO zicomtec_dataz.facturasped(empresa,sucursal,numerofac,fecha,cliente,porviva,vendedor,vendidopor,anulada,motivoanul,totalfactu,excentos,noexcentos,valoriva,saldo,servicio,notacredit,promocion,descuentos,porcdesc1,porcdesc2,bodega,actualizad,tipo,impreso,vence,fechacont,costog,costong,forpago,tipof,transporte,numerofacf,codprecio,centrocos,vidrio,observacion) 
	VALUES ('01','01','$numfacturacompleta','$created_at','',0,'$id_empleado','',0,'',$totcompra,$subnograva,$subgrava,$totiva,$totcompra,0,0,0,0,0,0,'',0,'F',0,null,null,0.00,0.00,'',0,0.00,'','','',0,'')");

$update=mysqli_query($con,"UPDATE maenum SET nentrega= '$purchase_order_id'+1 where 1");
...
if (!empty($factura))
{
	...
	$sql=mysqli_query($con, "select * from moddetfacturasped where moddetfacturasped.nrofact='$factura'");
$i=0;
...
var_dump(sizeof($product_id));
	...
	for($i=0;$i<sizeof($product_id);$i++){
	   ...
	   $insert2=mysqli_query($con,"INSERT INTO detfacturasped(empresa,sucursal,nrolinea,nrofactura,codigo,numordent,fechav,observa,bodega,
	   cantidad,cantidadc, preciouni,porcdesc1,porcdesc2,costotal,preciooc,lp,ancho,largo,saldo,tipo,iva,promocion,
	   ordent,tecnico,tiempo,tiempor,facturaot,pvp,factor,refvidrio) VALUES 
	    ('01','01',1,'$nrofact[$i]','$product_id[$i]','',null,null,'01',$cant[$i],0,$precio_unitario[$i],$pordesc[$i],0.00000000,$subtotal,0,'',0.00,0.00,$cant[$i],'F','1',0,0,''
