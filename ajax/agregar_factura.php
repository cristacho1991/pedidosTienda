...
if (isset($_POST['precio_venta'])) {$unit_price = floatval($_POST['precio_venta']);}
if (isset($_POST['descuento'])) {$descuento = floatval($_POST['descuento']);}
...
if (!empty($id) and !empty($qty) and !empty($unit_price)) {
	...
	$insert = mysqli_query($con, "INSERT INTO zicomtec_dataz.moddetfacturas(id,nrofact,idproducto,descripcion,cant,preciouni,subtotal,pordesc1)
...
<table class="table">
	...
	$totaliva = 0;
	$sumador_total = 0;
	$sql = mysqli_query($con, "select id,idproducto,descripcion,cant,preciouni,pordesc1,(select costos.cantidad from costos where costos.codstock=moddetfacturas.idproducto order by codstock desc limit 1) as disponible from moddetfacturas where nrofact='$numfacturacompleta'");
	while ($row = mysqli_fetch_array($sql)) {
		...
		$subtotal = (($unit_price * $qtyfact) - (($unit_price * $qtyfact) * ($descuento / 100)));
	...
	?>
		<tr>
			...
			<td class='text-center col-md-1'>
				<input type="number" style="text-align:center" disabled="" value="<?php echo $qty; ?>" class="form-control input-sm">
			</td>
			...
			<td class='col-md-1'>
				...
				<input type="text" style="text-align:right" class="form-control input-sm" id="stock_<?php echo $product_id; ?>" value="<?php echo $disponible; ?>" disabled="disabled">

			</td>
			<!--CANTIDAD FACTURADA-->
			<td class='text-center col-md-1'>
				<input type="number" style="text-align:center" value="<?php echo intval($qtyfact); ?>" class="form-control input-sm" onblur="actualizar_item(this.value,1,'<?php echo $product_id; ?>')">
			</td>
			...
			<!--DESCUENTO-->
			<td class="col-md-1">
				<input type="number" style="text-align:right" value="<?php echo $descuento; ?>" class="form-control input-sm" disabled="disabled" onblur="actualizar_item(this.value,3,'<?php echo $id; ?>')">
			</td>
...
<input class="btn btn-success" type="button" name="agregar_registros" value="Guardar Factura" onClick="guardar_factura();" />
...
if (confirm('Realmente desea cancelar esta factura?')) {
