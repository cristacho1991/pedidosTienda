<?php
	$next="SELECT `AUTO_INCREMENT` FROM  INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '".DB_NAME."' AND   TABLE_NAME   = 'tstock'";
	$query_next=mysqli_query($con,$next);
	$rw_next=mysqli_fetch_array($query_next);
	$next_insert=$rw_next['AUTO_INCREMENT'];
	
	$codstock=$next_insert;
	
	
	//echo "el codigo es ".$product_code;
	$created_at=date("Y-m-d H:i:s");
	//$target_dir="img/productos/product.png";
	$inser=mysqli_query($con,"INSERT INTO tstock (empresa,sucursal,codstock,nombre,alterno1,linfam,unimed,iva,costoprom,procedenci,ulcompra,cuentac) VALUES ('01','01','$codstock','', '', '', '', '', '', '', '', ''); ");
	//$sql_product=mysqli_query($con,"select * from products where  product_code='$product_code'");
	$sql_product=mysqli_query($con,"select * from tstock where  codstock='$codstock'");

	$rw_product=mysqli_fetch_array($sql_product);
	
	//$count=mysqli_query($con,"select count(*) as total from tstock where baja=0");
            ...
            <div class="tab-content">
              ...
              <div class="tab-pane active" id="details">
                  ...
                  <div class="form-group ">
                    <label for="product_code" class="col-sm-2 control-label">Código producto</label>

                    <div class="col-sm-4">
                      <input type="text" class="form-control" id="product_code" name="product_code"  value="<?php echo $codstock;?>"required >
					   <input type="hidden"  id="product_id" name="product_id"  value="<?php echo $codstock;?>" >
                    </div>
					<label for="codalt" class="col-sm-2 control-label">Código alterno</label>

                    <div class="col-sm-4">
                      <input type="text" class="form-control" id="codalt" name="codalt" >
                    </div>
                  </div>
				  ...
				  <div class="form-group">
					...
					<label for="linfam" class="col-sm-2 control-label">Linfam</label>

                    <div class="col-sm-4">
                      <input type="text" class="form-control" id="linfam" name="linfam" maxlength="100" >
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="unimed" class="col-sm-2 control-label">Unidad de Medida</label>
                    <div class="col-sm-4">
					<input type="text" class="form-control" name="unimed" id="unimed">
                    </div>
                   ...
                   <div class="form-group">
                    <label for="buying_price" class="col-sm-2 control-label">Costo Promedio</label>
