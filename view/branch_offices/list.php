...
<html>
  ...
  <body class="hold-transition <?php echo $skin;?> sidebar-mini">
	<?php 
		...
		include("modal/agregar_sucursal.php");
				...
				<div class="row">
					...
					<div class="col-xs-12 col-md-7">
						<div class="btn-group pull-right">
							...
							<button type="button" class="btn btn-default"  data-toggle="modal" data-target="#modal_register"><i class='fa fa-plus'></i> Nueva</button>
							<?php }?>
	...
	function per_page(valor){
		...
		function eliminar(id){
			if(confirm('Esta acción  eliminará de forma permanente la sucursal \n\n Desea continuar?')){
				var page=1;
				var query=$("#q").val();
				var per_page=$("#per_page").val();
...
$( "#new_register" ).submit(function( event ) {
 ...
 var parametros = $(this).serialize();
	 $.ajax({
			type: "POST",
			url: "ajax/registro/agregar_sucursal.php",
			data: parametros,
			 beforeSend: function(objeto){
...
$( "#update_register" ).submit(function( event ) {
 ...
 var parametros = $(this).serialize();
	 $.ajax({
			type: "POST",
			url: "ajax/modificar/sucursal.php",
			data: parametros,
			 beforeSend: function(objeto){
				$("#resultados_ajax").html("Enviando...");
			  },
...
<script>
  ...
  modal.find('.modal-body #code2').val(code)
  modal.find('.modal-body #name2').val(name)
  modal.find('.modal-body #address2').val(address)
  modal.find('.modal-body #phone2').val(phone)
